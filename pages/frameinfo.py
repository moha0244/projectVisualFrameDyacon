...
char_line = ''.join(
    f"<span style='display:inline-block;width:18px;text-align:center;'>{c}</span>" for c in trame
)

index_line = ''.join(
    f"<span style='display:inline-block;width:18px;text-align:center;font-size:10px;color:gray;'>{i % 100}</span>"
    for i in range(1, len(trame)+1)
)
